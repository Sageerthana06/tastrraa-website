...
export const initDb = async () => {
  ...
  if (!process.env.DATABASE_URL) {
    console.log(
      "ℹ️ No DATABASE_URL found. Running with in-memory fallback store.",
    );
    isPgConnected = false;
    return;
  }
  ...
  try {
    ...
    await client.query(
      `ALTER TABLE products ADD COLUMN IF NOT EXISTS unit VARCHAR(50) DEFAULT '1kg';`,
    );
    await client.query(
...
export const queryDb = async (text, params) => {
  if (isPgConnected) {
    try {
      ...
      console.error(
        "⚠️ PostgreSQL query error, falling back to memory store:",
        err.message,
      );
...
const executeMemoryQuery = (text, params = []) => {
  ...
  if (normalized.includes("insert into products")) {
    const newId = memoryProducts.length
      ? Math.max(...memoryProducts.map((p) => p.id)) + 1
      : 1;
    const [
      name,
      slug,
      ...
      memoryProducts[prodIndex] = {
        ...
        description:
          description !== undefined
            ? description
            : memoryProducts[prodIndex].description,
        category:
          category !== undefined
            ? category
            : memoryProducts[prodIndex].category,
        price:
          price !== undefined
            ? parseFloat(price)
            : memoryProducts[prodIndex].price,
        unit: unit !== undefined ? unit : memoryProducts[prodIndex].unit,
        image_url:
          image_url !== undefined
            ? image_url
            : memoryProducts[prodIndex].image_url,
        features:
          features !== undefined
            ? typeof features === "string"
              ? JSON.parse(features)
              : features
            : memoryProducts[prodIndex].features,
        is_active:
          is_active !== undefined
            ? is_active
            : memoryProducts[prodIndex].is_active,
        updated_at: new Date(),
      };
